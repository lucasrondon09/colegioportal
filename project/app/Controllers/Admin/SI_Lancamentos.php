...
class SI_Lancamentos extends BaseController
{
    ...
    /**
     * Gerar boleto para uma parcela
     */
    public function gerarBoleto($id)
    {
        try {
            // Carregar biblioteca de boletos
            $boletoService = new \App\Libraries\BoletoService();
            
            // Gerar boleto
            $resultado = $boletoService->gerarBoletoIndividual($id);
            
            if ($resultado['success']) {
                // Redirecionar para visualização do boleto
                return redirect()->to(base_url('Admin/Lancamentos/visualizarBoleto/' . $id))
    ...
    public function visualizarBoleto($id)
    {
        try {
            ...
            if (!$parcela) {
                return redirect()->back()
                    ->with('error', 'Parcela não encontrada');
            }
            
            // Verificar se boleto já foi gerado
            if (empty($parcela->nosso_numero)) {
                return redirect()->to(base_url('Admin/Lancamentos/gerarBoleto/' . $id))
                    ->with('info', 'Gerando boleto...');
            }
            
            // Carregar biblioteca de boletos
            $boletoService = new \App\Libraries\BoletoService();
            
            // Gerar boleto novamente para visualização
            $resultado = $boletoService->gerarBoletoIndividual($id);
            ...
            return view('Admin/SI_Lancamentos/visualizar_boleto', $data);
            
        } catch (\Exception $e) {
            log_message('error', 'Erro ao visualizar boleto: ' . $e->getMessage());
            return redirect()->back()
                ->with('error', 'Erro ao visualizar boleto: ' . $e->getMessage());
        }
    }
    
    /**
     * Imprimir/Download boleto em PDF
     */
    public function imprimirBoleto($id)
    {
        try {
            ...
            if (!$resultado['success']) {
                return redirect()->back()
                    ->with('error', 'Erro ao gerar boleto para impressão');
            }
            
            // Renderizar PDF
            $pdf = $boletoService->renderizarBoletoPDF($resultado['boleto_obj']);
            
            // Retornar PDF para download
            return $this->response
                ->setHeader('Content-Type', 'application/pdf')
                ->setHeader('Content-Disposition', 'inline; filename="boleto_' . $id . '.pdf"')
                ->setBody($pdf);
            
        } catch (\Exception $e) {
            log_message('error', 'Erro ao imprimir boleto: ' . $e->getMessage());
            return redirect()->back()
                ->with('error', 'Erro ao imprimir boleto: ' . $e->getMessage());
        }
    }
