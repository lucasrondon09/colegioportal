...
class SI_Contrato extends Controller
{
    ...
    public function lancamentosCadastrar($id_contrato)
    {
        ...
        // Carregar configurações de boleto padrão
        $configBoletoModel = new \App\Models\Admin\SI_ConfiguracoesBoletoModel();
        $this->data['configBoleto'] = $configBoletoModel->getConfigAtiva();
            ...
            if ($this->validation->withRequest($this->request)->run()) {
                ...
                for ($i = 0; $i < (int)$fields['parcelas']; $i++) {
                    ...
                    $lancamento = [
                        ...
                        'juros_percentual'       => $fields['juros_percentual'] ?? null,
                        'multa_percentual'       => $fields['multa_percentual'] ?? null,
                        'multa_apos_dias'        => $fields['multa_apos_dias'] ?? null,
                        'desconto_percentual'    => $fields['desconto_percentual'] ?? null,
                        'nao_receber_apos_dias'  => $fields['nao_receber_apos_dias'] ?? null,
                        'protestar_apos_dias'    => $fields['protestar_apos_dias'] ?? null,
                        'aceite'                 => $fields['aceite'] ?? null,
                        'tipo_titulo'            => $fields['tipo_titulo'] ?? null,
                        'mensagem_banco'         => $fields['mensagem_banco'] ?? null
                    ];
                    
                    $id_parcela = $this->parcelaModel->insert($lancamento);
                    
                    // Se marcou para registrar boletos, gerar automaticamente
                    if (!empty($fields['registrar_boletos']) && $id_parcela) {
                        try {
                            $boletoService = new \App\Libraries\BoletoService();
                            $boletoService->gerarBoletoIndividual($id_parcela);
                        } catch (\Exception $e) {
                            log_message('error', 'Erro ao gerar boleto para parcela ' . $id_parcela . ': ' . $e->getMessage());
                        }
                    }
                }

                $alert = 'success';
                if (!empty($fields['registrar_boletos'])) {
                    $message = 'Os lançamentos foram salvos e os boletos foram gerados com sucesso!';
                } else {
